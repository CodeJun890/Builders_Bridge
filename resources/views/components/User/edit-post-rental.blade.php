@extends('components.Layout.edit-post-rental_layout')
...
@section('body')
    <section class="body-cont">
        ...
        @endif
            ...
            <form action="{{route('update-my-post-rent.put', $rental->id)}}"  id="postRentalForm" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                            ...
                            <div class="col-md-8 col-12 me-1"  data-aos="fade-down"  data-aos-delay="400">
                                <div class="col-12 p-4">
                                    ...
                                    <div class="form mt-3">
                                        ...
                                        <input type="text" name="rental_item_name" id="rental_item_name" value="{{$rental->rental_item_name ?? ""}}" class="form-control" required>
                                    </div>
                                    <div class="form mt-3">
                                        <label for="rental_description" class="form-lable fw-bold mb-2">Description</label>
                                        <textarea name="rental_description" class="form-control" id="rental_description">{{$rental->rental_description}}</textarea>
                                    </div>
                                    <div class="form mt-3">
                                        <div class="col-12 d-flex justify-content-center align-items-center">
                                            <label for="rent_image_one" style="background: grey;" class="label-container col-4 d-flex justify-content-center align-items-center text-light rounded">
                                                <i class="bi bi-plus-circle-fill @if($rental->rent_image_one) d-none @endif"></i>
                                                <img id="preview_one" src="{{ $rental->rent_image_one ? asset('storage/' . $rental->rent_image_one) : '' }}" class="preview border border-1 @if(!$rental->rent_image_one) d-none @endif" />
                                            </label>
                                            <label for="rent_image_two" style="background: grey;" class="label-container col-4 mx-2 d-flex justify-content-center align-items-center text-light rounded">
                                                <i class="bi bi-plus-circle-fill @if($rental->rent_image_two) d-none @endif"></i>
                                                <img id="preview_two" src="{{ $rental->rent_image_two ? asset('storage/' . $rental->rent_image_two) : '' }}" class="preview border border-1 @if(!$rental->rent_image_two) d-none @endif" />
                                            </label>
                                            <label for="rent_image_three" style="background: grey;" class="label-container col-4 d-flex justify-content-center align-items-center text-light rounded">
                                                <i class="bi bi-plus-circle-fill @if($rental->rent_image_three) d-none @endif"></i>
                                                <img id="preview_three" src="{{ $rental->rent_image_three ? asset('storage/' . $rental->rent_image_three) : '' }}" class="preview border border-1 @if(!$rental->rent_image_three) d-none @endif" />
                                            </label>
                            ...
                            <div class="col-md-4 col-12 ms-1" >
                                <div class="col-12 p-4">
                                    ...
                                    <div class="form mt-3">
                                        ...
                                        <select name="rental_status" id="rental_status" value="{{$rental->rental_status}}" class="form-select text-capitalize" required>
                                                <option value="{{$rental->rental_status}}" hidden>{{$rental->rental_status}}</option>
                                ...
                                <div class="col-12 p-4 mt-3">
                                    ...
                                    <div class="form mt-3">
                                        @php
                                            ...
                                            $formattedTags = json_decode($rental->tags);
                                            $selectedTags =  $formattedTags;

                                        @endphp
                                        <label for="engineeringTags">Add Tags</label>
                                        <select id="editEngineeringTags" name="engineeringTags" data-placeholder="Select tags" data-max="2" multiple data-multi-select required>
                                            <option value="">Select a category</option>
                                            @foreach($options as $option)
                                                <option value="{{ $option }}" {{ in_array($option, $selectedTags) ? 'selected' : '' }}>{{ $option }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="col-12 p-4 mt-3">
                                    <div class="d-flex justify-content-between align-items-center">
                                        ...
                                        <div>
                                            <input type="checkbox" name="rent_price_negotiable" id="rent_price_negotiable" value="negotiable" @if($rental->rent_price == 'negotiable') checked @endif>
                                    ...
                                    <div class="form mt-3">
                                        ...
                                        <div class="input-group flex-nowrap">
                                            ...
                                            <input type="text" name="rent_price" id="rent_price" class="form-control" autocomplete="off" required
                                                   @if($rental->rent_price == 'negotiable') disabled @endif
                                                   value="{{ $rental->rent_price != 'negotiable' ? $rental->rent_price : '' }}">
