@extends('components.Layout.rental_layout')
    ...
    <section class="body-cont">
        ...
        <div class="body">
            ...
            <div class="d-flex justify-content-between align-items-center mt-3 pe-3 nav-responsive-breadcrumb" data-aos="fade-right"  data-aos-delay="200">
                ...
                <div class="create-rental-btn-cont">
                    <a href="{{route('publish-rental.get')}}" class="create-rental-btn btn btn-outline-success mb-2" ><i class="bi bi-plus-circle-fill me-1"></i>Rental Posting</a>
                </div>
            </div>
            <div class="create-rental-btn-responsive text-center">
                <a href="{{route('publish-rental.get')}}" class=" btn btn-sm btn-outline-success mb-2" ><i class="bi bi-plus-circle-fill me-1"></i>Rental Posting</a>
            </div>
            <div class="mt-3">
                    ...
                    <div class="row justify-content-center align-items-center px-3 " data-aos="fade-right" data-aos-delay="200">
                        <div class="col-12 d-flex align-items-center ">
                            ...
                            <div class="col-md-3 col-12 px-3 item-type">
                                @php
                                    // Collect all tags from rentals
                                    $allTags = collect();
                                    foreach ($rentals as $rental) {
                                        $tags = json_decode($rental->tags);
                                        if (is_array($tags)) {
                                ...
                                <input list="filter-tags" id="filter-tags-input" name="filter-tags" class="form-control" placeholder="Select Tags" autocomplete="off">
                        ...
                        <div id="rental-items-container" data-aos="fade-up" data-aos-delay="400">
                            @if ($rentals->isEmpty())
                                <div class="text-center mt-5">
                                    <div class="lead text-uppercase fw-bold text-secondary">No item posted yet <i class="bi bi-emoji-dizzy-fill"></i></div>
                                </div>
                            @else
                                @php
                                    $chunks = $rentals->chunk(5); // Chunk the rentals collection into groups of 5
                                @endphp
                                @foreach ($chunks as $chunk)
                                    <div class="col-12 rental-item-cont d-flex align-items-center mt-4 gap-3">
                                        @foreach ($chunk as $rental)
                                            <div class="card rental-item" style="width: 18rem;">
                                                <div class="position-relative border-bottom border-1 container-img">
                                                    ...
                                                    @endphp
                                                    <button class="button-container btn btn-primary" onclick="viewRentItem('{{ encrypt($rental->id)}}')"><i class="bi bi-eye-fill text-light me-1"></i> View More</button>
                                                    <img src="{{ asset($image) }}" class="card-img-top" alt="..." style="height: 200px; width: 100%;">
                                                </div>
                                                <div class="card-body position-relative">
                                                    @if ($rental->rent_price !== "negotiable")
                                                        <span class="fw-bold" style="font-size: 1.25rem; color: green;">₱ {{ $rental->rent_price }}</span>
                                                    @else
                                                        <span class="fw-bold text-capitalize" style="font-size: 1.25rem; color: red;">{{ $rental->rent_price }}</span>
                                                    @endif
                                                    <div style="position: absolute; top: 20px; right: 10px;">
                                                        @php
                                                            $saved = Auth::check() && Auth::user()->savedLists->contains('rental_id', $rental->id);
                                                        @endphp
                                                    ...
                                                    <p class="fw-bold lead m-0">{{ $rental->rental_item_name }}</p>
                                                    <span style="font-size: 0.75rem;"><i class="bi bi-geo-alt-fill text-danger" style="font-size: 1rem;"></i> {{ optional(json_decode($rental->ownerRental->addresses))->city }}, {{ optional(json_decode($rental->ownerRental->addresses))->province }}</span>
                                                    <hr class="mt-2">
                                                    <div class="d-flex justify-content-center">
                                                        <span class="mb-2 text-center w-100" style="font-size: 0.75rem;">Posted {{$rental->created_at->diffForHumans()}}</span>
                                                    </div>
